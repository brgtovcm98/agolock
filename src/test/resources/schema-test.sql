...
CREATE TABLE IF NOT EXISTS spaces (
    ...
    image_external_id UUID         REFERENCES images(external_id),
    created_at        TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- V8 마이그레이션 인덱스 (운영 환경과 실행 계획 동기화)
-- H2는 부분 인덱스(partial index) 문법을 지원하지 않으므로, 테스트에서는 일반 인덱스로 대체한다.
...
CREATE INDEX IF NOT EXISTS idx_stocks_space_id_status_shelf_null
    ON stocks (space_id, status, shelf_id);

CREATE INDEX IF NOT EXISTS idx_stocks_shelf_id_status_box_null
    ON stocks (shelf_id, status, box_id);
